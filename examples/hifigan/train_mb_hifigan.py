...
from examples.multiband_melgan.train_multiband_melgan import MultiBandMelganTrainer
...
class MultiBandHifiGANGeneratorConfig(HifiGANGeneratorConfig):
    """Initialize Multi-band HifiGAN Generator Config."""
...
def main():
    ...
    with STRATEGY.scope():
        ...
        generator = TFHifiGANGenerator(
            MultiBandHifiGANGeneratorConfig(**config["hifigan_generator_params"]),
        ...
        pqmf = TFPQMF(
            MultiBandHifiGANGeneratorConfig(
                **config["hifigan_generator_params"]
            ),
            dtype=tf.float32,
            name="pqmf",
        )
